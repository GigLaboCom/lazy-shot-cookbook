...
4. **Never annotate originals.** `add_markers` forks a copy by default; keep the original for diffing. Pass `edit_existing: true` only when the user explicitly asks to overwrite. Note what the copy actually is: a **marker layer** — coordinates and labels in metadata, with the same pixels as the source. Markers are never drawn into a file, not even on export. Say so when you hand over the path, and don't imply the user can flatten them: to get visible numbered badges in a publishable image, they place **Counter** annotations (shortcut `N`) in the app by hand.
