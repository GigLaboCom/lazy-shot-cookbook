...
```text
capture the window  →  keyword it  →  numbered markers (on a copy)  →  ship to docs
        ↑                                                                  │
...
3. add_markers with numbered markers on each UI element I should mention,
   in reading order. Remember: this forks a copy — reference the copy's
   file_path in your summary.

When all steps are done, output a Markdown snippet for our docs: one image
per step (the annotated copies' file paths) with a numbered caption list
...
- `add_markers` doesn't mutate: budget for two files per step (original + annotated copy).
